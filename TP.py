...
powerupImage = {'shroom': '/Users/joshuahuang/Desktop/112/TP/TP/shroom.png',
                'star': '/Users/joshuahuang/Desktop/112/TP/star.png'}
...
immune_frames_w = {0:["/Users/joshuahuang/Desktop/112/TP/walkingi.png","/Users/joshuahuang/Desktop/112/TP/walkingi2.png","/Users/joshuahuang/Desktop/112/TP/walkingi3.png","/Users/joshuahuang/Desktop/112/TP/walkingi4.png"],
             1:["/Users/joshuahuang/Desktop/112/TP/walkingbi1.png","/Users/joshuahuang/Desktop/112/TP/walkingbi2.png","/Users/joshuahuang/Desktop/112/TP/walkingbi3.png","/Users/joshuahuang/Desktop/112/TP/walkingbi4.png"]}   
immune_frames_j = {0:["/Users/joshuahuang/Desktop/112/TP/jumpingi.png",
                    "/Users/joshuahuang/Desktop/112/TP/jumpingi2.png"],
                    1:["/Users/joshuahuang/Desktop/112/TP/jumpingbi1.png",
                    "/Users/joshuahuang/Desktop/112/TP/jumpingbi2.png"]}                                 
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.immune = False
        self.coltime = pygame.time.get_ticks()
        self.last_update = 0
    ...
    def gravity(self,gravity = .35):
        ...
        if self.rect.y  >= height-55 and self.vspeed >=0:
            self.vspeed = 0
        ...
        if self.immune:
            if self.walking:
                if now - self.last_update > 100:
                    ...
                    self.current_framei = (self.current_framei + 1)%4
                    if self.hspeed > 0:
                        self.image = pygame.image.load(immune_frames_w[self.state]
                                                         [self.current_framei])
                        self.image = pygame.transform.scale(self.image,(23,23))
                        self.image.set_colorkey(white)
                    else:
                        self.image = pygame.image.load(immune_frames_w[self.state][self.current_frame])
            ...
            if self.jumping:
                if now - self.last_update > 100:
                    self.last_update = now
                    self.current_framei = (self.current_framei + 1)%2
                    self.image = pygame.image.load(immune_frames_j[self.state][self.current_frame])
                    self.image = pygame.transform.scale(self.image,(23,23))
                    self.image.set_colorkey(white)
        elif self.walking:
            if now - self.last_update > 200:
                ...
                if self.hspeed > 0:
                    ...
                    self.image = pygame.transform.scale(self.image,(23,23))
                    self.image.set_colorkey(white)
        elif self.jumping: #same for jumping
            if self.hspeed >= 0:
...
class Level2(Level):
    def __init__(self):
        ...
        b61= Block2(2557,height-99,17,17)
        b62= Block2(2572,height-99,17,17)
        b63= Block2(2587,height-99,17,17)
        b64= Block2(2602,height-99,17,17)
        b65= Block2(2617,height-99,17,17)
        b66= Block2(2632,height-99,17,17)
        b82= Block2(2649,height-99,17,17)
        b83= Block2(2649,height-99,17,17)
        b84= Block2(2666,height-99,17,17)
        ...
        self.mario.set_position(135,height-60)
        self.players.add(self.mario)
        self.powerupblocks.add(qb1)
        self.starblocks.add(qb5)
...
if (__name__ == "__main__"):
    ...
    start = time.time()
    while running:
        if pygame.time.get_ticks() - curLevel.mario.coltime > 4000: 
            curLevel.mario.immune = False
        for event in pygame.event.get():
            if event.type == KEYDOWN:
                if event.key == K_q:  #keypress on q ends the game 
                    quit()
                if event.key == K_UP: # w,a,d to move/jump
                    curLevel.mario.jump()
        ...
        #immunity from http://stackoverflow.com/questions/17119464/how-to-make-an-object-have-temporary-invincibility
        #checks if mario was hit by lava 
        lavahit = pygame.sprite.spritecollide(curLevel.mario,curLevel.lavas,
                                              False,pygame.sprite.collide_circle)
        if lavahit:
            if not curLevel.mario.immune:
                curLevel.mario.state -= 1
                curLevel.mario.changeState(curLevel.mario.state)
                curLevel.mario.immune = True
                curLevel.mario.animate()
                curLevel.mario.coltime = pygame.time.get_ticks()
        ...
        for enemy in enemies:
            if curLevel.mario.immune:
                ...
                curLevel.collide.remove(enemy)
                curLevel.enemies.remove(enemy)
            elif curLevel.mario.rect.y >= enemy.rect.y:
                if not curLevel.mario.immune:
                    curLevel.mario.state -= 1
                    curLevel.mario.changeState(curLevel.mario.state)
                    curLevel.mario.immune = True
        ...
        for enemy in enemies:
            if curLevel.mario.immune:
                ...
                curLevel.collide.remove(enemy)
                curLevel.enemies.remove(enemy)
            if curLevel.mario.rect.y < enemy.rect.y and enemy.state < 0:
        ...
        starbs = pygame.sprite.spritecollide(curLevel.mario, curLevel.starblocks,
                                            False,pygame.sprite.collide_circle)
        for hit in starbs:
            if hit.hit == False and curLevel.mario.rect.y > hit.rect.y:
                hit.hit = True
                p1 = PowerUp("star",hit.rect.x,hit.rect.y-18, 18,18)
                curLevel.collide.add(p1)
                curLevel.allsprites.add(p1)
                curLevel.powerups.add(p1)
